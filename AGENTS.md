...
- Checks: `pnpm typecheck` (tsc), `pnpm lint` (oxlint `--type-aware`), `pnpm format:check` (oxfmt), `pnpm validate:content` (MDX and evidence), `pnpm test:unit` (geometry), `pnpm test:e2e` (Playwright). `pnpm check` runs the full suite including `build`.
