...
This is a static/SSR portfolio site built with TanStack Start (Vite). There is no backend, database, or external service to run — the contact form submits via a `mailto:` link, so no server-side form handling exists. Standard scripts live in `package.json`; setup is documented in `README.md` (macOS/Mise) and `mise.toml`.
